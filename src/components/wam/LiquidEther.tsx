...
export default function LiquidEther({
  ...
  // Use color0, color1, color2 if provided to build the colors array
  const finalColors = React.useMemo(() => {
    if (color0 || color1 || color2) {
      const arr = [];
      if (color0) arr.push(color0);
      if (color1) arr.push(color1);
      if (color2) arr.push(color2);
      return arr;
    }
    return colors;
  ...
  useEffect(() => {
    ...
    const paletteTex = makePaletteTexture(finalColors);
